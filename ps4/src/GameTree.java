...
public class GameTree {
   ...
   /*public void fillPlayer(TreeNode node, int x) {
       if(x==0) {
           node.nextPlayer = x;
           for (int i = 0; i < node.numChildren; i++) {
               fillPlayer(node.children[i], 1);
           }
       }
       else {
              node.nextPlayer=1;
              for(int i=0;i<node.numChildren;i++) {
                  fillPlayer(node.children[i],0);
              }
           }

       } */

    //Fills value and nextPlayer
    public void fillValue(TreeNode x,int y) {
       x.nextPlayer = y;
       ...
       if(x.leaf) {
           ...
           if(x.nextPlayer==1) {
               ...
               for(int i=0;i<x.numChildren;i++) {
                   fillValue(x.children[i],0);
               }
               for(int i=0;i<x.numChildren;i++) {
                   best = (best>=x.children[i].value) ? best : x.children[i].value;
               }
              x.value = best;
           }
           else {
               ...
               for(int i=0;i<x.numChildren;i++) {
                   fillValue(x.children[i],1);
               }
               for(int i=0;i<x.numChildren;i++) {
                   worst = (worst<=x.children[i].value) ? worst : x.children[i].value;
               }
